...
import { BODY_TYPES, cmToFt, ftInToCm } from '../lib/format';
...
const empty: FormState = { name: '', birthday: '', ft: '', inch: '', body_type: '', notes: '', extra: [] };
...
export default function ProfileEdit() {
  ...
  async function submit(e: FormEvent) {
    ...
    setError('');
    setBusy(true);
    const height_cm = form.ft || form.inch ? ftInToCm(Number(form.ft || 0), Number(form.inch || 0)) : null;
          ...
          <div className="space-y-2">
            {form.extra.map((row, i) => (
              <div key={i} className="flex gap-2">
                <input placeholder="Label (e.g. Eyes)" className={input} value={row.key}
